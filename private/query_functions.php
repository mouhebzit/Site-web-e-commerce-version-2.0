<?php
  ...
  function find_all_products() {
    ...
    $sql .= "WHERE Produit.Quantite > 0";
    $result = mysqli_query($db, $sql);
    confirm_result_set($result);
    return $result;
  }

  function find_all_products_price_asc() {
    global $db;
  ...
  function find_all_products_price_desc() {
    global $db;
  ...
  function find_all_products_name_asc() {
    global $db;
  ...
  function find_all_products_name_desc() {
    global $db;

    $sql = "SELECT * FROM Produit ";
    $sql .= "WHERE Produit.Quantite > 0";
    $sql .= " Order By Nom_produit DESC";
    $result = mysqli_query($db, $sql);
    confirm_result_set($result);
    return $result;
  }
  ...
  function find_produit_by_id($id) {
    global $db;
  ...
  function update_produit($subject) {
    global $db;

    $errors = validate_produit($subject);
    if(!empty($errors)) {
      return $errors;
    }

    $sql = "UPDATE produit SET ";
    $sql .= "ID_prod='" . db_escape($db, $subject['ID_prod']) . "', ";
    $sql .= "Type_produit='" . db_escape($db, $subject['Type_produit']) . "', ";
    $sql .= "Nom_produit='" . db_escape($db, $subject['Nom_produit']) . "', ";
    $sql .= "Taille='" . db_escape($db, $subject['Taille']) . "', ";
    $sql .= "Marque='" . db_escape($db, $subject['Marque']) . "', ";
    $sql .= "Couleur='" . db_escape($db, $subject['Couleur']) . "', ";
    $sql .= "Description='" . db_escape($db, $subject['Description']) . "', ";
    $sql .= "Quantite='" . db_escape($db, $subject['Quantite']) . "', ";
    $sql .= "Image='" . db_escape($db, $subject['Image']) . "', ";
    $sql .= "Prix='" . db_escape($db, $subject['Prix']) . "' ";
    $sql .= "WHERE id='" . db_escape($db, $subject['ID_prod']) . "' ";
    $sql .= "LIMIT 1";
  ...
  function validate_produit($user) {
    $errors = [];
    ...
    if(is_blank($user['nom'])) {
      $errors[] = "Name cannot be blank.";
    } elseif(!has_length($user['nom'], ['min' => 2, 'max' => 255])) {
      $errors[] = "Name must be between 2 and 255 characters.";
    }
    ...
    } elseif(!has_length($user['description'], ['min' => 2, 'max' => 255])) {
      $errors[] = "Name must be between 2 and 255 characters.";
    }
    if(is_blank($user['couleur'])) {
      $errors[] = "Content cannot be blank.";
    } elseif(!has_length($user['couleur'], ['min' => 2, 'max' => 255])) {
      $errors[] = "Name must be between 2 and 255 characters.";
    }

    if(is_blank($user['marque'])) {
      $errors[] = "Content cannot be blank.";
    } elseif(!has_length($user['marque'], ['min' => 2, 'max' => 255])) {
      $errors[] = "Name must be between 2 and 255 characters.";
    }

    if(is_blank($user['type'])) {
      $errors[] = "Content cannot be blank.";
    } elseif(!has_length($user['type'], ['min' => 2, 'max' => 255])) {
      $errors[] = "Name must be between 2 and 255 characters.";
    }
    if(is_blank($user['quantite'])) {
      $errors[] = "Content cannot be blank.";
    }
  ...
  function insert_admin($user) {
    ...
    $sql = "INSERT INTO users ";
    $sql .= "(first_name, last_name, email, username, admin_access ,hashed_password) ";
    $sql .= "VALUES (";
    $sql .= "'" . db_escape($db, $user['first_name']) . "',";
    $sql .= "'" . db_escape($db, $user['last_name']) . "',";
    $sql .= "'" . db_escape($db, $user['email']) . "',";
